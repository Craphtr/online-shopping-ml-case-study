...
def preprocessor_shopping(cleaned_shopping,
                          numeric_to_cat=True,
    ...
    ##Handle Missing and unexpected values in month bfore mapping
    ...
    if month_to_numeric:
        if not pd.api.types.is_numeric_dtype(preprocessed_shopping["month"]):            
            month_mapping = {"Feb": 2, "Mar": 3, "May": 5, "June": 6, "Jul": 7, "Aug": 8, "Sep": 9, "Oct": 10, "Nov": 11, "Dec": 12}
            preprocessed_shopping["month"] = preprocessed_shopping["month"].map(month_mapping)
    ...
    if encoder:
        cleaned_and_prepd_shopping = pd.get_dummies(preprocessed_shopping, dtype="float")
    ...
    print('Number of null values after preprocessing is',cleaned_and_prepd_shopping.isnull().sum())    
